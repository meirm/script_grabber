...
function setupFilters() {
  ...
  const archivedStatusFilter = document.getElementById('archived-status-filter') as HTMLSelectElement;
 ...
 */
async function navigateToJobDetails(jobId: string, isArchived: boolean = false) {
...
function setupBulkArchiveHandlers() {
  const selectAllCheckbox = document.getElementById('select-all-jobs') as HTMLInputElement;
  const bulkArchiveBtn = document.getElementById('bulk-archive-btn')!;
  const jobsList = document.getElementById('jobs-list')!;

  // Handle select all checkbox
  selectAllCheckbox.addEventListener('change', () => {
    const checkboxes = document.querySelectorAll('.job-checkbox') as NodeListOf<HTMLInputElement>;
...
function updateBulkArchiveButton() {
  const checkboxes = document.querySelectorAll('.job-checkbox:checked') as NodeListOf<HTMLInputElement>;
  const bulkArchiveBtn = document.getElementById('bulk-archive-btn') as HTMLButtonElement;
 ...
 */
async function handleJobArchive(jobId: string) {
...
async function handleBulkArchive() {
  ...
  try {
    ...
    showMessage(
      messageDiv,
      'success',
      `Archived ${response.archived_count} jobs successfully. ${response.failed_count} failed.`
    );
 ...
 */
async function showJobScript(jobId: string, isArchived: boolean) {
 ...
 */
async function showJobScriptInDetails(jobId: string, isArchived: boolean) {
 ...
 */
async function handleStatusUpdate(jobId: string, action: 'stale' | 'failed') {
  const messageDiv = document.getElementById('upload-status')!;
